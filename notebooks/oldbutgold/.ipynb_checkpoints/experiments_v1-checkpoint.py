...
def run_experiment(data_folder, model_name, batch_size, epochs, W=256, inputs=None, loss_type=None, filt_alt=None, num=0, \
    ...
    # Add index to the dataset for filtering
    dataset_with_index = dataset.enumerate()
    outlier_mask = np.load('../data/external/outliers_mask.npy')
    normal_mask = ~outlier_mask  # Logical NOT to identify normal images
    normal_mask_tensor = tf.constant(normal_mask, dtype=tf.bool)
    outlier_mask_tensor = tf.constant(outlier_mask, dtype=tf.bool)

    try:
        if augment:
            # Separate normal and outlier datasets
            normal_dataset = dataset_with_index.filter(
                lambda idx, _: tf.gather(normal_mask_tensor, idx)
            ).flat_map(
                lambda idx, inputs_label: tf.data.Dataset.from_tensor_slices(
                    map_augment(inputs_label[0], inputs_label[1], idx, inputs_label[2], is_outlier=False)
                    if conditioned else
                    map_augment(inputs_label[0], inputs_label[1], idx, is_outlier=False)
                ).map(lambda image, label: (tf.convert_to_tensor(image), tf.convert_to_tensor(label)))
            )
            
            outlier_dataset = dataset_with_index.filter(
                lambda idx, _: tf.gather(outlier_mask_tensor, idx)
            ).flat_map(
                lambda idx, inputs_label: tf.data.Dataset.from_tensor_slices(
                    map_augment(inputs_label[0], inputs_label[1], idx, inputs_label[2], is_outlier=True)
                    if conditioned else
                    map_augment(inputs_label[0], inputs_label[1], idx, is_outlier=True)
                ).map(lambda image, label: (tf.convert_to_tensor(image), tf.convert_to_tensor(label)))
            )

        
            # Combine augmented datasets
            dataset = normal_dataset.concatenate(outlier_dataset)
    ...
    if augment:
        ...
        tf.print("Sample Image Shape:", tf.shape(sample_image))
        tf.print("Sample Label Shape:", tf.shape(sample_label))
        
        augmented_images = [
            augment_data(sample_image[0], sample_label[0])[0][0].numpy()
            if not conditioned
            else augment_data(sample_image[0][0], sample_label[0])[0][0].numpy()
            for _ in range(10)
        ]
        ...
        normal_augmented_images_count = count_images(normal_dataset)  # Assuming `augmented_dataset` contains augmented images
        outlier_augmented_images_count = count_images(outlier_dataset)
        ...
        with open(output_file, "w") as f:
            ...
            f.write(f"Augmented normal images: {normal_augmented_images_count}\n")
            f.write(f"Augmented outliers: {outlier_augmented_images_count}\n")
            f.write(f"Total images (original + augmented): {normal_augmented_images_count + outlier_augmented_images_count}\n")
    ...
    for epoch in range(epochs):
        ...
        num_batches = 0
        start_idx = 0
        dataset = dataset.shuffle(buffer_size=len(train_model_input[0]))
    ...
    augmented_images_count = normal_augmented_images_count + outlier_augmented_images_count
